...
namespace MotorcycleTourManagment.Booking
{
    public partial class frm_NewBooking : Form
    {
        ...
        private void btn_Next_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in grd_Customer.Rows)
                if (rows.Contains(row.Index))
                    CustomerIDs.Add((int)row.Cells[0].Value);
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            int TourID = (int)grd_Tours.Rows[row].Cells[0].Value;
            dbConn = new DatabaseConnection();
            foreach (int CustomerID in CustomerIDs)
                dbConn.SendCommand("INSERT INTO tbl_Booking (CustomerID, TourID) VALUES ('" + CustomerID.ToString() + "', '" + TourID.ToString() + "');");
            dbConn.Close();
            this.Close();
        }
